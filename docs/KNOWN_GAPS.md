...
| A3 | **Stereo calibration** with the real mounted cameras, then `--verify` against a tape measure | [CALIBRATION.md](CALIBRATION.md) | Everything depth-related is uncalibrated garbage until this is done |
| A4 | **Confirm camera discovery hints** — check `v4l2-ctl --list-devices` shows names containing `AR0234` / `IMX477`; adjust `stereo_name_hint`/`wide_name_hint` in `argus.yaml` if your units report different strings | `argus/cameras.py` | If both AR0234s AND the wide cam report identical names, discovery falls back to resolution grouping — verify it picked correctly in `selftest` |
...
| A7 | **End-to-end latency + memory profiling** under `tegrastats` (fast-loop Hz, wake→answer time, RAM headroom) | — | Tune `depth.fast_downscale`, `agent.ctx_size`, image_max_side from measurements |
